...
- **`react-native-worklets`'s Babel plugin needs `@babel/{core,generator,traverse,types}` declared via `packageExtensions`** (in `pnpm-workspace.yaml`). The worklets plugin (loaded by `babel-preset-expo` during `expo export --platform web`, which the production build runs) does `require("@babel/traverse")` etc. but does NOT declare them. Under pnpm strict linking they're otherwise only resolvable via fragile peer-variant hoisting, which is **non-deterministic across install environments** — so the production build failed *intermittently* with `Cannot find module '@babel/traverse'` (some publishes succeeded, others failed with identical code). The `packageExtensions` block forces pnpm to hard-link these into worklets' own `node_modules` in every install. Do NOT remove it. After editing, run `pnpm install`; verify with `ls artifacts/ks-solar/node_modules/react-native-worklets` resolving to a variant whose sibling `node_modules` contains `@babel/traverse`.
...
- **Run `eas build` with `EAS_NO_VCS=1`.** EAS's default project-archive path runs git operations (it touches `.git/index.lock`), which are blocked in this environment (`Destructive git operations are not allowed`). `EAS_NO_VCS=1 eas build --profile preview --platform android --non-interactive --no-wait` archives the working directory directly and uploads fine. `--no-wait` returns a build URL immediately since cloud builds take ~15-25 min.
- **Expo account / build token.** The project is now owned by the Expo account **`kns43`** (knssolarenergy43@gmail.com), `extra.eas.projectId` = `149196a9-1c6e-46f3-b5df-189a465ecdc1`, `owner: "kns43"` in `app.json`. Builds authenticate with the **`EXPO_TOKEN_ALT`** shared env var, so prefix every EAS command with `EXPO_TOKEN="$EXPO_TOKEN_ALT"` (e.g. `EXPO_TOKEN="$EXPO_TOKEN_ALT" EAS_NO_VCS=1 eas build ...`). The original account `knssolars-team` (old projectId `c6bb79cb-...`, token in `EXPO_TOKEN`) exhausted its Free-plan monthly Android builds, which is why we switched. A new project means a **new Android keystore is generated on first build** — devices that have an APK signed by the old `knssolars-team` keystore must **uninstall it first** before installing a `kns43`-signed APK (Android rejects a signature change). After the first build, the `kns43` keystore is reused (no interactive prompt).
- The **`preview` build profile's `EXPO_PUBLIC_DOMAIN` points at the production deploy URL**, so the standalone APK talks to the *production* API — the server must be deployed with current code for the APK to work end-to-end. Admin env vars are set as **shared** so they apply to production automatically.
